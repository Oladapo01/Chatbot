...
from keras.models import load_model                  ## loading the model that the train.py produced
...
def clean_sentence(sentence):                    # cleaning the sentences
...
def words_box(sentence, show_details=True):     # giving values for each word
...
def prediction(sentence):        # computes the likelihood of the chosen class
    ...
    predict_list = []
    for r in results:
        predict_list.append({'intent': classes[r[0]], 'probability': str(r[1])})
    return predict_list
